...
static const char* wifi_sec_type_2_string(WIFISecurity_t sec_type)
{
 ...
 * Description  : Converts an extended WiFi security type (WIFISecurityExt_t)
 *                into a standard WiFi security type (WIFISecurity_t). This
 *                reduction is required because mobile provisioning clients
 *                (Android/iOS) do not support extended security enumerations.
 * Arguments    : WIFISecurityExt_t sec_type -
 *                  Extended WiFi security type received from provisioning.
 * Return Value : WIFISecurity_t -
 *                  Corresponding reduced WiFi security type. Returns
 *                  eWiFiSecurityNotSupported for unsupported or unknown types.
 ******************************************************************************/
...
#define MAX_WIFI_SCAN_RESULTS 20
    ...
    if (eWiFiSuccess == rc )
    {
        ...
        for (i = 0; i < num_of_found_networks; i++)
        {
            ...
            if ((eWiFiSecurityNotSupported == security) ||
...
static void wifi_network_params_print(const WIFINetworkParams_t* params)
{
	LOG_INFO("WIFINetworkParams_t: {\n"
             ...
             "}\n", sizeof(params->ucSSID), params->ucSSID, params->ucSSIDLength,
                    ...
                    wifi_sec_type_2_string(params->xSecurity), params->ucDefaultWEPKeyIndex, params->ucChannel
    );
}
...
static int_t wifi_ext_connect(const prov_cmd_select_ap_t* params)
{
    ...
    switch (security)
    {
        ...
        default:
        {
            APP_PRINT_ERR("Unable to connect to WiFi network due to "
                "unsupported security type: %s (%d)\n", wifi_sec_type_2_string(security), security);
    ...
    {
        if(FSP_SUCCESS != store_provisioning_params(p_net_params))
        {
