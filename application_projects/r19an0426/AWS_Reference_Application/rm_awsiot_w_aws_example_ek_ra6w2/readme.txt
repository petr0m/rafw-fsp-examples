...
6. Verification:
 ...
 7.  Modify the default AWS Thingname if needed, using the cli command, nvram setenv appcfg thingname <THINGNAME>.
 8.  Do the wifi provisioning using the Renesas Wifi Provisioning application from google play store or ios appstore.
 9.  Select "WiFi+BLE combo chip" tab for provisioning. Select the device name, "RRQ-DEVICE" from the BLE scan list.
 ...
 16. After one lock/unlock action is completed, the AWS application is ready to go to sleep and ble svc_stop command is
     set by AWS application to BLE to stop BLE. Now the device will go to sleep. 
 17. Only after RA6W2 goes to sleep, do the next lock/unlock toggle action.
 18. When the lock/unlock is done from the mobile application, the terminal logs will show the door lock state changes
     accordingly.
