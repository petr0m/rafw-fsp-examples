...
void atcmd_mcu_wakeup(uint8_t port, uint16_t pin)
{
    ...
    R_GPIO_W_PinCfg(&g_ioport_ctrl, l_pin
        , ((uint32_t) IOPORT_PERIPHERAL_GPIO
        | (uint32_t) IOPORT_CFG_PORT_DIRECTION_OUTPUT
        | (uint32_t) IOPORT_CFG_PORT_OUTPUT_HIGH));
    vTaskDelay(1);
    R_GPIO_W_PinWrite(&g_ioport_ctrl, l_pin, BSP_IO_LEVEL_LOW);
}
...
int app_iot_at_push_com_parser(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        errorCode = FSP_ERR_AT_CMD_ERR_INSUFFICENT_ARGS;
...
int32_t app_at_check_certi_info(void)
{
    ...
    rm_cert_err_t cert_ret = RM_CERT_ERR_EMPTY_CERTIFICATE;
    ...
    cert_ret = RM_CERT_Read(RM_CERT_GetModule(SF_TLS_CERT_ATCMD_CA_ADDR),
                            RM_CERT_TYPE_CA_CERT, &format, (unsigned char *) temp_cert, &outlen);
    printf("\nRoot CA: %s\n", cert_ret == RM_CERT_ERR_OK ? "O" : "X");
    if (cert_ret != RM_CERT_ERR_OK)
        ret = 0;

    outlen = CERT_MAX_LENGTH;
    cert_ret = RM_CERT_Read(RM_CERT_GetModule(SFLASH_ATCMD_TLS_CERT_01),
                            RM_CERT_TYPE_CERT, &format, (unsigned char *) temp_cert, &outlen);
    printf("Certificate: %s\n", cert_ret == RM_CERT_ERR_OK ? "O" : "X");
    if (cert_ret != RM_CERT_ERR_OK)
        ret = 0;

    outlen = CERT_MAX_LENGTH;
    cert_ret = RM_CERT_Read(RM_CERT_GetModule(SFLASH_ATCMD_TLS_CERT_02),
                            RM_CERT_TYPE_PRIVATE_KEY, &format, (unsigned char *) temp_cert, &outlen);
...
RM_ATCMD_W_CORE_AWS_UNFIXED_ATCMD_CB(CERT)
{
    ...
    if ((ch < 0x30) || (ch > 0x39))
    {
        err = FSP_ERR_AT_CMD_ERR_INSUFFICENT_ARGS;
        goto end;
    }
    cert_id = ch - 0x30;
    ...
    if (err == FSP_ERR_AT_CMD_ERR_CMD_OK)
    {
        ...
        switch (cert_id)
        {
            case 0:
                /* CA */
                if (RM_CERT_Write(RM_CERT_GetModule(SF_TLS_CERT_ATCMD_CA_ADDR),
            ...
            case 1:
                /* Certificate */
                if (RM_CERT_Write(RM_CERT_GetModule(SFLASH_ATCMD_TLS_CERT_01),
                                  RM_CERT_TYPE_CERT, RM_CERT_FORMAT_PEM, p_cert, cdata_idx))
                {
                    err = FSP_ERR_AT_CMD_ERR_SFLASH_WRITE;
                }
            break;
            case 2:
                /* Key */
                if (RM_CERT_Write(RM_CERT_GetModule(SFLASH_ATCMD_TLS_CERT_02),
                                  RM_CERT_TYPE_PRIVATE_KEY, RM_CERT_FORMAT_PEM, p_cert, cdata_idx))
                {
                    err = FSP_ERR_AT_CMD_ERR_SFLASH_WRITE;
                }
            break;
