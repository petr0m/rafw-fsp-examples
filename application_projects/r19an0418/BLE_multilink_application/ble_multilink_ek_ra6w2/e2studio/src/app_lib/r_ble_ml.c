...
#endif
#if AUTOTEST_ENABLE
static bool security_level_printed;
#endif
...
static void print_connections (void)
{
    ...
    APP_PRINT("\r\n");
    APP_PRINT("Nr | Index | Address\r\n");
    R_Queue_Foreach(&connections, print_connection_func, &num);

    if (!num)
    {
        APP_PRINT("(no active connections)\r\n");
    }
...
#if CONN_WORKAROUND
    APP_PRINT("\tConnection index: %u\r\n", idx);
#else
    APP_PRINT("\tConnection index: %u\r\n", evt->conn_hdl);
#endif
    APP_PRINT("\tAddress: %s %s\r\n",
...
static bool handle_disconnect (st_ble_gap_disconn_evt_t * evt)
{
    ...
    if (master_dev_conn_hdl == evt->conn_hdl)
    {
        ...
        APP_PRINT("Advertising is on again\r\n");
        start_adv = true;
    }
...
static bool handle_conn_cancel_comp (ble_status_t result)
{
    ...
    if (BLE_SUCCESS == result)
    {
        if (CONN_CANCEL_REASON_DEV_NOT_RESPONDING == conn_canceled_reason)
        {
            ...
            APP_PRINT("handle_evt_gap_conn_completed: Status=%d\r\n", err);
    ...
    else
    {
        if (BLE_GAP_INVALID_CONN_HDL == master_dev_conn_hdl)
        {
            APP_PRINT("Undefined cancellation reason\r\n");
            start_adv = true;
        }
    }
...
static void handle_adv_off (st_ble_gap_adv_off_evt_t * evt)
{
    ...
    APP_PRINT("Advertising completed\r\n");
    APP_PRINT("\tAdvertising type: 1\r\n");
    APP_PRINT("\tCompletion status: 0x%02x\r\n", status);
}
