...
static int tcp_client_keepalive_conf(int socket_fd, int enable, int keep_idle, int keep_intvl, int keep_cnt)
{
    ...
    if (keep_idle)
    {
        ...
        if (ret)
        {
            printf("Failed to set TCP_KEEPIDLE (%d)\n", ret);
        }
    ...
    if (keep_intvl)
    {
        ...
        if (ret)
        {
            printf("Failed to set TCP_KEEPINTVL (%d)\n", ret);
        }
    ...
    if (keep_cnt)
    {
        ...
        if (ret)
        {
            printf("Failed to set TCP_KEEPCNT (%d)\n", ret);
        }
    ...
    if (keep_idle)
    {
        ...
        if (ret)
        {
            printf("Failed to set TCP_KEEPALIVE\n");
        }
...
static void wait_ready_connect_ipv6()
{
    ...
    if (ip6_addr_isglobal(&peer_ip6addr))
    {
        ...
        while (!ip6_addr_isvalid(netif_ip6_addr_state(&wlan0_iface, 1)))
        {
            ...
            if (wait_cnt > 100)
            {
                printf("Global IPv6 is not valid, attempting to connect with Link-local IPv6 address\n");
                break;
            }
...
void tcp_client_dpm_task(void * pvParams)
{
    ...
    printf("Start of TCP Client application\n");
    ...
    if (ip_type == IPADDR_TYPE_V4)
    {
        ...
        if (socket_fd < 0)
        {
            printf("Failed to assign socket descriptor\n");
            goto end_of_task;
        }
    ...
    if (ret < 0)
    {
        printf("Failed to establish TCP session(%d,%d)\n", ret, errno);
        close(socket_fd);
        goto end_of_task;
    }
    ...
    while (1)
    {
        ...
        if (g_tcpc_send_cnt > 0)
        {
            ...
            {
                printf("Sending tcp data (%s) done !!\n", p_send_buf);
            }
        ...
        else
        {
            if (errno != EAGAIN)
            {
                printf("Failed to receive data(%d,%d), closing tcp client ... \n", len, errno);
                /* Either TCPC is disconnected or WI-Fi is disconnected */
                break;
            }
