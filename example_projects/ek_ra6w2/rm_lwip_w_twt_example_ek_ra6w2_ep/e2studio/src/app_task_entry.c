...
static void wifi_event_handler(WIFIEvent_t * pxEvent)
{
...
static void twt_event_callback(WIFIEvent_t *pxEvent)
{
    switch (pxEvent->xEventType)
    {
        ...
        {
            WIFIEventExt_t *pxExtEvent = (WIFIEventExt_t *)pxEvent;
...
static void wifi_twt_config(void)
{
    ...
    {
        len = RM_PMGR_W_user_rtm_get(TWT_RTM_NAME, (unsigned char **)&p_twt);

        if (len == 0)
        {
            ret = (int)RM_PMGR_W_user_rtm_pool_alloc(TWT_RTM_NAME,(void **)&p_twt, sizeof(twt_rtm_t), 0);
            if (ret)
            {
                printf("[APP] Failed to allocate RTM for TWT(%d)\n", ret);
                p_twt = NULL;
            }
            else
            {
...
void app_task_entry(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (!p_twt->twt_session_active)
        {
            ...
            while ((!p_twt->twt_session_active) && (twt_setup_retry_cnt < 3))
            {
                twt_setup_retry_cnt++;
