...
2) Add SSID and Password of the Target AP to connect in the src\config.h file.
3) Generate and build the example project.
4) Flash the EP project to the RA6W2 board with the Flashing Procedure.
5) After flashing, press Reset button.
6) Make sure target AP have internet connection, since SNTP need to fetch time from the server. 
7) Check the current time in the Segger/RTTViewer.
