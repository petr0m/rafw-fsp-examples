...
#define OS_TASK_NOTIFY_APP_BITS             (ALERT_TMO_NOTIF | BAS_TMO_NOTIF | BLINK_TMO_NOTIF | PXP_UPDATE_CONN_PARAM_NOTIF)
...
static uint8_t gs_advertising_data[] =
{
    ...
    0x0A, /**< Data Size */
    0x09, /**< Data Type */
    0x50, 0x58, 0x50, 0x2d, 0x52, 0x41, 0x36, 0x57, 0x32, /**< Data Value */
};
...
#endif
            if (2 != p_gap_adv_evt_param->reason)
            {
            	uint8_t i;
                /* restart ADV only if not max connected devices */
                for (i = 0; i < CONN_SUPPORTED_MAX_NUM; i++)
        		{
...
/* Start user code for process before main loop. Do not edit comment generated here */
    ...
    uint32_t notif;
    g_lls_alert_timer = xTimerCreate("lls timer", 15 * 1000 / TICK_PERIOD_MS,
                                 pdFALSE, xTaskGetCurrentTaskHandle(), (TimerCallbackFunction_t  ) lls_alert_timeout_cb);
#if (PX_REPORTER_INCLUDE_BAS == 1)
    g_bas_timer = xTimerCreate("bas timer", PX_REPORTER_BATTERY_CHECK_INTERVAL / TICK_PERIOD_MS,
                                 pdTRUE, xTaskGetCurrentTaskHandle(), (TimerCallbackFunction_t  ) bas_timeout_cb);
#endif
    /* Create timer to implement led blinking */
    g_led_blink_timer = xTimerCreate("led", 500 / TICK_PERIOD_MS, pdFALSE, xTaskGetCurrentTaskHandle(),
                                 led_blink_tim_cb);
#if AUTOTEST_ENABLE
        ...
        if (notif & PXP_UPDATE_CONN_PARAM_NOTIF)
        {
            ...
            if (p_conn_dev && p_conn_dev->expired)
            {
                ...
                if (gp_param_connections)
                {
                	...
                	xEventGroupSetBitsFromISR((EventGroupHandle_t)g_ble_event_group_handle, (EventBits_t) PXP_UPDATE_CONN_PARAM_NOTIF,
...
static void bas_update(void)
{
     ...
     */
    status = R_GPIO_W_PinRead(&g_ioport_ctrl, BSP_IO_PORT_00_PIN_09, &ctrl_pin_status);
...
static void lls_alert_timeout_cb(TimerHandle_t xTimer)
{
	...
	BaseType_t pxHigherPriorityTaskWoken = false;
	xEventGroupSetBitsFromISR((EventGroupHandle_t)g_ble_event_group_handle, (EventBits_t) ALERT_TMO_NOTIF, &pxHigherPriorityTaskWoken);
}
...
static void bas_timeout_cb(TimerHandle_t xTimer)
{
	FSP_PARAMETER_NOT_USED(xTimer);
	BaseType_t pxHigherPriorityTaskWoken = false;
	xEventGroupSetBitsFromISR((EventGroupHandle_t)g_ble_event_group_handle, (EventBits_t) BAS_TMO_NOTIF, &pxHigherPriorityTaskWoken);
}
#endif
...
static void led_blink_tim_cb(TimerHandle_t xTimer)
{
	FSP_PARAMETER_NOT_USED(xTimer);
	BaseType_t pxHigherPriorityTaskWoken = false;
	xEventGroupSetBitsFromISR((EventGroupHandle_t)g_ble_event_group_handle, (EventBits_t) BLINK_TMO_NOTIF, &pxHigherPriorityTaskWoken);
}
...
static void conn_params_timer_cb(TimerHandle_t xTimer)
{
    ...
    p_conn_dev = R_List_find(gp_param_connections, conn_params_match,
                                            (const void *) (uint32_t) p_conn_dev->conn_hdl);
    if (p_conn_dev)
    {
...
static void handle_gap_conn_ind(st_ble_gap_conn_evt_t *evt)
{
    ...
    if (p_conn_dev)
    {
        ...
        p_conn_dev->param_timer = xTimerCreate("conn timer",
...
static void handle_gap_disconn_ind(st_ble_gap_disconn_evt_t *evt)
{
    conn_dev_t *p_conn_dev = R_List_unlink(&gp_param_connections, conn_params_match,
    ...
    if (gs_connected_device_info[evt->conn_hdl].level_update)
    {
        ...
        {
    		lls_alert_update(evt->conn_hdl, gs_connected_device_info[evt->conn_hdl].remote_addr, gs_connected_device_info[evt->conn_hdl].level);
            return;
        }
    }
    else
