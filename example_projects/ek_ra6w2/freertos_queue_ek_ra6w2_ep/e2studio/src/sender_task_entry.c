    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (pdPASS == xQueueReset(g_queue))
            {
                ...
                fsp_err = R_TIM_W_Stop( &g_periodic_timer_msgq_ctrl);
                /* Handle error */
                if (FSP_SUCCESS != fsp_err)
                {
                    if (FSP_SUCCESS != R_TIM_W_Close( &g_periodic_timer_msgq_ctrl))
                    {
                ...
                fsp_err = R_TIM_W_Close( &g_periodic_timer_msgq_ctrl);
                if (FSP_SUCCESS != fsp_err)
                {
                    /* Print out in case of error */
                    APP_ERR_PRINT("\r\nSender_Task : GPT Timer Close API failed\r\n");
