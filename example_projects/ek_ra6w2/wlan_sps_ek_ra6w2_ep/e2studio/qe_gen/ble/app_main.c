...
#include "rm_comms_ble.h"
#include "rm_comms_lock.h"
#include "rm_comms_bridge.h"
#include "rm_ble_msg_queue.h"
...
/* Start user code for GAP callback function event process. Do not edit comment generated here */
		...
		{
			const st_ble_gap_data_len_chg_evt_t * p_gap_data_len_chg_evt_param = (st_ble_gap_data_len_chg_evt_t *) p_data->p_param;
		...
		{
			const st_ble_gap_pairing_req_evt_t *p_gap_pairing_req_evt_param = (st_ble_gap_pairing_req_evt_t *)p_data->p_param;
		...
		{
			const st_ble_gap_pairing_info_evt_t * p_gap_pairing_info_evt_param = (st_ble_gap_pairing_info_evt_t *)p_data->p_param;
...
/* Start user code for SPS Service Server callback function event process. Do not edit comment generated here */
        ...
        {
            st_ble_sps_services_sps_server_rx_data_t * p_rx_data = (st_ble_sps_services_sps_server_rx_data_t *) p_data->p_param;

            rm_comms_ble_callback(RM_COMMS_BLE_EVENT_DATA_AVAILABLE, p_rx_data->rx_data, p_rx_data->rx_len, g_comms_ble.p_ctrl);
        } break;
        ...
        if (RM_BLE_MSG_QUEUE_Dequeue(p_input_msg) == FSP_SUCCESS)
        {
            switch (p_input_msg->type)
            {
                ...
                case RM_BLE_MSG_TYPE_SVC_STOP:
                {
                    ...
                    while(APP_STATE_RUNNING != g_app_state)
                    {
                    	if (RM_BLE_MSG_QUEUE_Dequeue(p_input_msg) == FSP_SUCCESS)
                    	{
