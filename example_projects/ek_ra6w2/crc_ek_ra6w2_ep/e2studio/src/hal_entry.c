...
static fsp_err_t set_led(uint32_t onoff)
{
    ...
    if (onoff == LED_ON)
    {
        err = R_GPIO_W_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_10, BSP_IO_LEVEL_HIGH);
    }
    else if (onoff == LED_OFF)
    {
        err = R_GPIO_W_PinWrite(&g_ioport_ctrl, (bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_10, BSP_IO_LEVEL_LOW);
    }
    else
    {
