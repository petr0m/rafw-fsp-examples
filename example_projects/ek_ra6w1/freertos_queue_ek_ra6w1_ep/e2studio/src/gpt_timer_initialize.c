 ...
 **********************************************************************************************************************/
fsp_err_t gpt_timer_init(timer_ctrl_t *p_timer_ctrl, const timer_cfg_t *p_timer_cfg)
{
    fsp_err_t fsp_err = FSP_SUCCESS;

    /* Open GPT timer instance */
    fsp_err = R_TIM_W_Open(p_timer_ctrl, p_timer_cfg);
    /* Handle error */
    ...
    fsp_err = R_TIM_W_Start(p_timer_ctrl);
    /* Handle error */
    if (FSP_SUCCESS != fsp_err)
    {
        /* Close timer if failed to start */
        if (FSP_SUCCESS != R_TIM_W_Close(p_timer_ctrl))
        {
            /* Print out in case of error */
