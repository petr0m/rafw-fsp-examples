...
void semaphore_task_entry(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        {
            /* Stop the GPT timer */
            fsp_err = R_TIM_W_Stop( &g_periodic_timer_sem_ctrl);
            /* Handle error */
            if (FSP_SUCCESS != fsp_err)
            {
                if (FSP_SUCCESS != R_TIM_W_Close( &g_periodic_timer_sem_ctrl))
                {
            ...
            fsp_err = R_TIM_W_Close( &g_periodic_timer_sem_ctrl);
            if (FSP_SUCCESS != fsp_err)
            {
                /* Print out in case of error */
                APP_ERR_PRINT("\r\nSemaphore Task : GPT Timer Close API failed\r\n");
